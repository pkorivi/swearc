...
class Arduino:
  ...
  def setThresholds(self,forwardVal, seatVal, floorVal):
    try:
      self.ser.write("set.thresholds "+str(forwardVal)+","+str(seatVal)+","+str(floorVal)+"\n")
      return 1
    except:
      print("setThresholds: Failed to write to arduino")
      return 0

  def getThresholds(self):
    try:
      self.ser.flushInput()
      self.ser.write("get.thresholds\n")
      tmpArray = array.array('B', self.ser.readline().rstrip())
      return tmpArray
    except:
      print("getThresholds: Failed to write to arduino")
      return 0
